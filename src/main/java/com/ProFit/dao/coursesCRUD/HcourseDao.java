...
public class HcourseDao implements IHcourseDao {
	...
	public boolean updateCourseById(CourseBean newCourse) {
        ...
        oldCourse.setCourseName(
            newCourse.getCourseName() == null || newCourse.getCourseName().isEmpty() ? 
            oldCourse.getCourseName() : newCourse.getCourseName()
        );

        oldCourse.setCourseCreateUserId(
            newCourse.getCourseCreateUserId() == null || newCourse.getCourseCreateUserId().isEmpty() ? 
            oldCourse.getCourseCreateUserId() : newCourse.getCourseCreateUserId()
        );

        oldCourse.setCourseCategory(
            newCourse.getCourseCategory() == null || newCourse.getCourseCategory().isEmpty() ? 
            oldCourse.getCourseCategory() : newCourse.getCourseCategory()
        );

        oldCourse.setCourseInformation(
            newCourse.getCourseInformation() == null || newCourse.getCourseInformation().isEmpty() ? 
            oldCourse.getCourseInformation() : newCourse.getCourseInformation()
        );

        oldCourse.setCourseDescription(
            newCourse.getCourseDescription() == null || newCourse.getCourseDescription().isEmpty() ? 
            oldCourse.getCourseDescription() : newCourse.getCourseDescription()
        );

        oldCourse.setCourseEnrollmentDate(
            newCourse.getCourseEnrollmentDate() == null || newCourse.getCourseEnrollmentDate().isEmpty() ? 
            oldCourse.getCourseEnrollmentDate() : newCourse.getCourseEnrollmentDate()
        );

        oldCourse.setCourseStartDate(
            newCourse.getCourseStartDate() == null || newCourse.getCourseStartDate().isEmpty() ? 
            oldCourse.getCourseStartDate() : newCourse.getCourseStartDate()
        );

        oldCourse.setCourseEndDate(
            newCourse.getCourseEndDate() == null || newCourse.getCourseEndDate().isEmpty() ? 
            oldCourse.getCourseEndDate() : newCourse.getCourseEndDate()
        );

        oldCourse.setCoursePrice(
            newCourse.getCoursePrice() == null || newCourse.getCoursePrice().isEmpty() ? 
            oldCourse.getCoursePrice() : newCourse.getCoursePrice()
        );

        oldCourse.setCourseStatus(
            newCourse.getCourseStatus() == null || newCourse.getCourseStatus().isEmpty() ? 
            oldCourse.getCourseStatus() : newCourse.getCourseStatus()
        );
