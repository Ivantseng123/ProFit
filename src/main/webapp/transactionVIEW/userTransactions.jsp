...
<body>
    ...
    <main>
        ...
        <div class="modal fade" id="updateModal" tabindex="-1" aria-labelledby="updateModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="post" action="${pageContext.request.contextPath}/UserTransactionServlet" id="updateTransactionForm" onsubmit="return confirm('確定要更新這筆交易嗎？');">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            用戶ID: <input type="text" id="updateUserId" name="user_id" required class="form-control"><br>
                            交易類型: <select id="updateTransactionType" name="transaction_type" required class="form-select">
                                <option value="deposit">存入</option>
                                <option value="withdrawal">取出</option>
                                <option value="payment">付款</option>
                                <option value="refund">退款</option>
                            </select><br>
                            金額: <input type="text" id="updateAmount" name="amount" required class="form-control"><br>
                            交易狀態: <select id="updateTransactionStatus" name="transaction_status" required class="form-select">
                                <option value="pending">待處理</option>
                                <option value="completed">已完成</option>
                                <option value="failed">失敗</option>
                            </select><br>
                            創建時間: <input type="text" id="updateCreatedAt" name="created_at" class="form-control" readonly><br>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
                ...
                <tbody id="transactionBody">
                    <c:forEach var="transaction" items="${transactions}">
                        <tr id="transactionRow${transaction.transactionId}">
                            ...
                            <td><fmt:formatDate value="${transaction.completionAt}" pattern="yyyy-MM-dd HH:mm:ss" /></td>

                            <td>
                                <!-- 顯示更新和刪除按鈕，當狀態為 'pending' 時才顯示 -->
                                <c:if test="${transaction.transactionStatus == 'pending'}">
                                    ...
                                    <form method="post" action="${pageContext.request.contextPath}/UserTransactionServlet" style="display:inline;" class="deleteTransactionForm" onsubmit="return confirm('確定要刪除這筆交易嗎？');">
