					...
					<form method="post"
						...
						<div class="modal-body">
							...
							<input type="text" id="addUserId" name="user_id" required
								class="form-control"><br> <br> 交易類型: <select
								id="addTransactionType" name="transaction_type" required
								class="form-select">
								<option value="deposit">存入</option>
								<option value="withdrawal">取出</option>
								<option value="payment">付款</option>
								<option value="refund">退款</option>
							</select><br> <br> 金額: <input type="text" id="addAmount"
								name="amount" required class="form-control"><br> <br>
						</div>
					...
					<form method="post"
						...
						<div class="modal-body">
							<input type="hidden" name="action" value="applyUpdate"> <input
								type="hidden" name="transaction_id" id="updateTransactionId">

							用戶ID: <input type="text" id="updateUserId" name="user_id"
								required class="form-control"><br> 交易類型: <select
								id="updateTransactionType" name="transaction_type" required
								class="form-select">
								<option value="deposit">存入</option>
								<option value="withdrawal">取出</option>
								<option value="payment">付款</option>
								<option value="refund">退款</option>
							</select><br> 金額: <input type="text" id="updateAmount" name="amount"
								required class="form-control"><br> 交易狀態: <select
								id="updateTransactionStatus" name="transaction_status" required
								class="form-select">
								<option value="pending">待處理</option>
								<option value="completed">已完成</option>
								<option value="failed">失敗</option>
							</select><br> 創建時間: <input type="text" id="updateCreatedAt"
								name="created_at" class="form-control" readonly><br>
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-secondary"
								data-bs-dismiss="modal">取消</button>
					...
					<c:forEach var="transaction" items="${transactions}">
						<tr id="transactionRow${transaction.transactionId}">
							...
							<td class="text-center">
								<!-- 顯示更新和刪除按鈕，當狀態為 'pending' 時才顯示 --> <c:if
