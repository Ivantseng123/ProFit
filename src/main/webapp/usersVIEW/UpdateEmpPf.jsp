...
<body>
    <jsp:include page="../model/header&sidebar.jsp"></jsp:include>
     <main>
		...
		<form class="form-container" method="post" action="UpdateEmpPf" enctype='multipart/form-data'>
					<input type="hidden" name="company_photoURL" value="${emp.company_photoURL}">				
					<label class="form-label" for="employer_profile_id"> 企業資訊ID: </label>
						<input
							class="form-input" type="text"
							id="employer_profile_id" name="employer_profile_id" value="${emp.employer_profile_id}" readonly>  									
					<label class="form-label" for="user_id"> 會員ID: </label> 
						<input
							class="form-input" type="text"
							id="user_id" name="user_id" value="${emp.user_id}" readonly>  							
					<label class="form-label"for="company_name">公司名稱:</label> 
						<input class="form-input" type="text" placeholder="修改公司名稱" 
						id="company_name" name="company_name" value="${emp.company_name}" required>
						
					<label class="form-label" for="company_address">公司地址:</label>
						<select name="company_address" id="company_address" class="address-option">
						...
						<input
						class="form-input" type="text" placeholder="修改公司地址"
						id="company_address" name="company_address1" value="${fn:substring(emp.company_address, 3, fn:length(emp.company_address))}
" required> 
					...
					<label class="form-label" for="company_phoneNumber">公司電話:</label> 
						<input class="form-input-insert" type="text" placeholder="輸入公司電話" 
							id="company_phoneNumber" name="company_phoneNumber" value="${emp.company_phoneNumber}" required>					
					<label class="form-label"for="company_taxID">公司統編:</label> 
						<input class="form-input" type="text" placeholder="修改公司統編" 
						id="company_taxID" name="company_taxID" value="${emp.company_taxID}" required>	
												
					<label class="form-label"for="company_numberOfemployee">公司員工數:</label>					
        				<select id="numberOfemployee" class="numOfemp-option" name="company_numberOfemployee" style="margin-bottom: 20px">
					...
					<label class="form-label"for="company_captital">公司資本額:</label>
					<input class="form-input" type="text" placeholder="修改公司資本額" 
						id="company_captital" name="company_captital" value="${emp.company_captital}">		
					<label class="form-label" for="company_description">公司簡介:</label> 
						<textarea rows="10" cols="50" id="company_description" name="company_description">${emp.company_description}</textarea>					   														
					<label class="form-label"for="nationalID-img">公司照片:</label>
					<div class="company-picture" style="">						
        				<img class="company-img" id="companyImgPreview" src="${pageContext.request.contextPath}/usersVIEW/userupload/${emp.company_photoURL}" alt="公司照片"/>      
				    </div>
				    <div>		
						<input class="file-uploader_companyImg" type="file" name="companyPhoto"
							accept="image/*"/>
					</div>								  										
	...
	<script>
		$(function() {
			...
			let category = "${emp.company_category}";
			...
			let numOfemp = "${emp.company_numberOfemployee}";
			...
			let address = "${fn:substring(emp.company_address, 0, 3)}";
